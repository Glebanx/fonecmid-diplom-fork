...
Функция  ВКМ_ПолучитьРеализацииИДоговоры(Период) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	ДоговорыКонтрагентов.ВКМ_ДействуетС <= НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	               |	И ДоговорыКонтрагентов.ВКМ_ДействуетПо >= КОНЕЦПЕРИОДА(&Период, МЕСЯЦ)
...
Функция ВКМ_СоздатьНовыйДокументРеализации(СтрокаМассива)
	...
	НовыйДокумент.Организация = СтрокаМассива.Организация;
	НовыйДокумент.Контрагент = СтрокаМассива.Владелец;
...
Процедура ЗаполнитьТЧ(Договор, Реализация, ТЧ) Экспорт
	...
	НоваяСтрокаТБ.ВКМ_Реализации = Реализация;
